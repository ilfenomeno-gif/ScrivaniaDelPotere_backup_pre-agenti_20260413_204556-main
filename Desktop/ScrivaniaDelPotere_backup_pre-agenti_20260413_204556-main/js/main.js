document.addEventListener('DOMContentLoaded', function () {
    ...
    const homeDlcEnableAllBtn = document.getElementById('home-dlc-enable-all');
    const homeDlcDisableAllBtn = document.getElementById('home-dlc-disable-all');
    ...
    const homeStoreFilters = document.getElementById('home-store-filters');
    ...
    function playPurchaseFeedback(btn) {
        if (!btn || !btn.classList) return;
        btn.classList.remove('purchase-impact');
        // Trigger a fresh animation frame to restart the effect even on rapid clicks.
        void btn.offsetWidth;
        btn.classList.add('purchase-impact');
        const timer = Number(btn.dataset.purchaseTimer || 0);
        if (timer) window.clearTimeout(timer);
        const nextTimer = window.setTimeout(() => {
            btn.classList.remove('purchase-impact');
            btn.dataset.purchaseTimer = '';
        }, 420);
        btn.dataset.purchaseTimer = String(nextTimer);
    }
    ...
    function renderDlcList() {
        ...
        homeDlcList.innerHTML = DLC_CATALOG.map((d, idx) => {
            const active = !!user.dlcState[d.id];
            const isNew = !active && d.type === 'Expansion';
            const badgeLabel = active ? 'Posseduto' : (isNew ? 'Nuovo' : 'Bloccato');
            const badgeClass = active ? 'badge-owned' : (isNew ? 'badge-new' : 'badge-locked');
            return `
                <div class="home-list-item dlc-card reveal-stagger" style="--stagger-index:${idx};">
                    <h3 class="home-list-title">${d.title}</h3>
                    <div class="home-list-tag">${d.desc}</div>
                    <div class="home-list-item-actions">
                        <span class="dlc-badge ${badgeClass}">${badgeLabel}</span>
    ...
    function setAllDlcState(active) {
        updateCurrentUser((u) => {
            if (!u.dlcState) u.dlcState = {};
            DLC_CATALOG.forEach((d) => {
                u.dlcState[d.id] = !!active;
            });
        });
        syncActiveDlcToGameState();
        renderDlcList();
        setHomeMessage(active ? 'Tutti i DLC sono stati attivati.' : 'Tutti i DLC sono stati disattivati.');
    }
    ...
    function renderStoreDetail(id) {
        ...
        homeStoreDetail.innerHTML = `
            <h3>${dlc.title}</h3>
            <p class="shop-item-price">${formatEuro(DLC_PRICE_MAP[dlc.id])}</p>
            ...
            <div class="home-store-card-actions">
                <button class="home-action-btn primary btn-buy" data-store-toggle-id="${dlc.id}">${active ? 'Disattiva DLC' : 'Acquista e Attiva DLC'}</button>
            </div>
        `;

        homeStoreDetail.querySelectorAll('[data-store-toggle-id]').forEach((btn) => {
            btn.addEventListener('click', () => {
                playPurchaseFeedback(btn);
    ...
    function renderStore() {
        ...
        homeStoreBundles.innerHTML = DLC_BUNDLES.map((b, idx) => {
            const allActive = b.ids.every((id) => !!user.dlcState[id]);
            return `
                <div class="home-store-bundle reveal-stagger" style="--stagger-index:${idx};">
                    <div class="home-store-title">${b.name}</div>
                    <div class="home-store-price shop-item-price">${formatEuro(b.price)}</div>
                    ...
                    <div class="home-store-bundle-actions">
                        <button class="home-action-btn primary btn-buy" data-store-bundle-id="${b.id}">${allActive ? 'Bundle già attivo' : 'Acquista e Attiva Bundle'}</button>
                    </div>
                </div>
            `;
        }).join('');
        ...
        const filteredCatalog = selectedStoreFilter === 'all'
            ? DLC_CATALOG
            : DLC_CATALOG.filter((d) => d.type === selectedStoreFilter);

        const grouped = filteredCatalog.reduce((acc, d) => {
        ...
        homeStoreList.innerHTML = Object.keys(grouped).map((type) => {
            const items = grouped[type].map((d, idx) => {
                const active = !!user.dlcState[d.id];
                const selected = selectedStoreDlcId === d.id;
                return `
                    <div class="home-store-card reveal-stagger${selected ? ' active' : ''}" style="--stagger-index:${idx};">
                        <div class="home-store-title">${d.title}</div>
                        <div class="home-store-price shop-item-price">${formatEuro(DLC_PRICE_MAP[d.id])}</div>
                        <div class="home-store-status">${active ? 'Attivo' : 'Non attivo'}</div>
                        <div class="home-store-mini-desc">${d.desc}</div>
                        <div class="home-store-card-actions">
                            <button class="home-action-btn" data-store-open-id="${d.id}">Dettagli</button>
                            <button class="home-action-btn btn-buy" data-store-quick-toggle-id="${d.id}">${active ? 'Disattiva' : 'Acquista'}</button>
                        </div>
                    </div>
                `;
            }).join('');
        ...
        if (!Object.keys(grouped).length) {
            homeStoreList.innerHTML = '<p class="home-list-tag">Nessun DLC disponibile per questo filtro.</p>';
        }

        const firstFiltered = filteredCatalog[0] && filteredCatalog[0].id;
        const fallbackId = firstFiltered || (DLC_CATALOG[0] && DLC_CATALOG[0].id);
        renderStoreDetail(selectedStoreDlcId || fallbackId);
    }
    ...
    homeDlcEnableAllBtn && homeDlcEnableAllBtn.addEventListener('click', () => {
        setAllDlcState(true);
    });

    homeDlcDisableAllBtn && homeDlcDisableAllBtn.addEventListener('click', () => {
        setAllDlcState(false);
    });
    ...
    homeStoreFilters && homeStoreFilters.addEventListener('click', (e) => {
        const btn = e.target.closest('[data-store-filter]');
        if (!btn) return;
        selectedStoreFilter = btn.getAttribute('data-store-filter') || 'all';
        homeStoreFilters.querySelectorAll('[data-store-filter]').forEach((filterBtn) => {
            filterBtn.classList.toggle('active', filterBtn.getAttribute('data-store-filter') === selectedStoreFilter);
        });
        selectedStoreDlcId = null;
        renderStore();
    });
