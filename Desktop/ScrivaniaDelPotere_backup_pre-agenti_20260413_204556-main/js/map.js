...
const GameMap = {
    ...
    async loadAllCities() {
        const oldWorldEnabled = (typeof Nations !== 'undefined'
            && typeof Nations.isOldWorldExpansionActive === 'function'
            && Nations.isOldWorldExpansionActive());

        if (this.allCitiesData && this._allCitiesOldWorldFlag === oldWorldEnabled) {
        ...
        this._allCitiesOldWorldFlag = oldWorldEnabled;
        return this.allCitiesData;
    },
    ...
    async renderCitySelection(containerId, onSelect, options) {
        ...
        container.innerHTML = `
            <div class="city-select-header">
                ...
                <div id="city-select-warning" class="city-select-warning" style="display:none"></div>
            </div>
        ...
        const cityWarning = document.getElementById('city-select-warning');

        const refreshUI = async () => {
            const visible = await getVisibleCities();
            let effectiveCities = visible;
            let warningText = '';

            if (currentMode === 'nation' && Object.keys(effectiveCities || {}).length === 0) {
                effectiveCities = await this.loadAllCapoluoghi();
                warningText = 'Nessun centro abitato disponibile per la nazione selezionata: visualizzo i capoluoghi disponibili.';
            }

            currentCities = effectiveCities;
            if (this.debugCitySelection) {
                console.info('[CitySelection] refresh', {
                    mode: currentMode,
                    nation: currentNation,
                    onlyCapoluoghi: showOnlyCapoluoghi,
                    visibleCities: Object.keys(currentCities || {}).length,
                });
            }
            this._renderCityList(currentCities, onSelect);
            this._renderCityMap(currentCities, onSelect, currentMode, currentNation, preferredCityId);

            if (cityWarning) {
                cityWarning.textContent = warningText;
                cityWarning.style.display = warningText ? 'block' : 'none';
            }

            if (citySearchSuggestions) {
                const options = new Set();
                Object.values(currentCities || {}).forEach(city => {
