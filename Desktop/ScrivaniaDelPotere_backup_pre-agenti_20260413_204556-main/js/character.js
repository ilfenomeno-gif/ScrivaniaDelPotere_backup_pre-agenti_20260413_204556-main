...
const Character = {
    ...
    getMentorBonusText(mentorId) {
        ...
        return this.getMentorBonusTextFromEntry(mentor);
    },

    getMentorBonusTextFromEntry(mentor) {
        if (!mentor) return 'Nessun bonus';

        if (mentor.bonusText) return mentor.bonusText;

        const bonus = mentor.bonus || mentor.effects;
        ...
        const bonusItems = Object.entries(bonus)
            .filter(([_, val]) => typeof val === 'number' && val > 0)
            .map(([key, val]) => `+${val} ${bonusLabels[key] || key}`);

        return bonusItems.join(', ') || 'Nessun bonus';
    },
    ...
    getAvailableMentors() {
        const ideology = this.normalizeIdeologyKey(Game.state.character.ideology);
    ...
    normalizeIdeologyKey(ideology) {
        const map = {
            ...
            'sinistra_radicale': 'estrema-sinistra',
            'sinistra': 'estrema-sinistra',
            'centro_sinistra': 'centro',
            'centro': 'centro',
            'centro_destra': 'centro',
            'destra': 'estrema-destra',
            'destra_radicale': 'estrema-destra',
        };
    ...
    async getAvailableMentorsForNation() {
        ...
        if (!this._mentorsPartiesDB) {
            try {
                ...
                if (!resp || !resp.ok) {
                    throw new Error('mentors-parties.json not reachable');
                }
        ...
        const ideologyOrder = ['radical_left', 'left', 'center_left', 'center', 'center_right', 'right', 'radical_right'];
        const selectedIdx = ideologyOrder.indexOf(ideology);

        const mappedMentors = (nationData.mentors || []).map((m, index) => {
            const mappedIdeology = this.mapPartyIdeologyToGame(m.ideology);
            const mappedIdx = ideologyOrder.indexOf(mappedIdeology);
            const distance = (selectedIdx < 0 || mappedIdx < 0) ? 99 : Math.abs(mappedIdx - selectedIdx);
            const normalizedMentor = this.normalizeNationMentorEntry(
                {
                    ...m,
                    shortName: m.shortName || m.short_name || m.name,
                    quote: m.quote || m.description,
                    bonusText: m.bonusText || this.getMentorBonusTextFromEntry(m),
                    effects: m.effects || m.bonus || {},
                    ideology: mappedIdeology,
                },
                ideology,
                this.MENTOR_AUTONOMO,
                index,
                Game.state.nation || { id: nationId, name: nationData.nation_name || nationId }
            );
            return { mentor: normalizedMentor, mappedIdeology, distance };
        });

        const exactMentors = mappedMentors
            .filter((x) => x.mappedIdeology === ideology)
            .map((x) => x.mentor);
        if (exactMentors.length > 0) {
            return [...exactMentors, this.MENTOR_AUTONOMO];
        }

        const ranked = mappedMentors
            .slice()
            .sort((a, b) => {
                if (a.distance !== b.distance) return a.distance - b.distance;
                const aName = String((a.mentor && (a.mentor.short_name || a.mentor.shortName || a.mentor.name)) || '');
                const bName = String((b.mentor && (b.mentor.short_name || b.mentor.shortName || b.mentor.name)) || '');
                return aName.localeCompare(bName);
            });

        const nearby = ranked.filter((x) => x.distance <= 1).map((x) => x.mentor);
        const fallbackMentors = (nearby.length ? nearby : ranked.map((x) => x.mentor)).slice(0, 6);
        if (fallbackMentors.length === 0) {
            return this.getAvailableMentors();
        }

        return [...fallbackMentors, this.MENTOR_AUTONOMO];
    },
