...
const Game = {
    ...
    checkGameOver() {
        ...
        if (s.stanchezza >= 100) {
            if (flags.lastExhaustionRescueDay !== this.state.day) {
                flags.lastExhaustionRescueDay = this.state.day;
                s.stanchezza = 92;
                s.stress = Math.min(100, (s.stress || 0) + 8);
                s.morale = Math.max(0, (s.morale || 0) - 6);
                this.addWorkNotif('🆘 Crollo Evitato', 'Sei arrivato al limite della stanchezza. Hai evitato il collasso, ma devi rallentare subito.', `Giorno ${this.state.day}`);
                this.emit('stat-change', { stat: 'stanchezza', old: 100, value: s.stanchezza, delta: -8 });
                return;
            }
            this.triggerGameOver('Sei crollato per la stanchezza. Non hai più energia per andare avanti.');
        } else if (s.stress >= 100) {
            if (flags.lastStressRescueDay !== this.state.day) {
                flags.lastStressRescueDay = this.state.day;
                s.stress = 92;
                s.morale = Math.max(0, (s.morale || 0) - 8);
                s.salute = Math.max(0, (s.salute || 0) - 4);
                this.addWorkNotif('🆘 Crollo Mentale Evitato', 'Hai sfiorato il burnout. Ti sei salvato per un soffio: prenditi una pausa.', `Giorno ${this.state.day}`);
                this.emit('stat-change', { stat: 'stress', old: 100, value: s.stress, delta: -8 });
                return;
            }
